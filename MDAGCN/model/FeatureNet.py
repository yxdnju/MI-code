...
class FeatureNet(nn.Module):
    def __init__(self, channels=10):
        ...
        self.ecg_weight = nn.Parameter(torch.ones(1))
    ...
    def get_feature(self, x):
        ...
        B, C, T = x.shape
        features = []
        weights = torch.softmax(torch.stack([self.eeg_weight, self.eog_weight, self.emg_weight, self.ecg_weight]), dim=0)

        for i in range(C):
            x_i = x[:, i, :].unsqueeze(1)

            # Modal classification for each channel
            if i <= 5:  # EEG (F3, F4, C3, C4, O1, O2)
                f_out = self.eeg_stream(x_i) * weights[0]
            elif i <= 7:  # EOG (ROC, LOC)
                f_out = self.eog_stream(x_i) * weights[1]
            elif i == 8:  # EMG
                f_out = self.emg_stream(x_i) * weights[2]
            else:  # ECG/Other
                f_out = self.eeg_stream(x_i) * weights[3]
    ...
    def forward(self, x):
        ...
        features = self.get_feature(x)  # [B, 10, 128]
